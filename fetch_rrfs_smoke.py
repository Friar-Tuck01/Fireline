...
# Western US bounds — matches OVERLAY_BOUNDS in index.html. wgrib2's -new_grid
# expects longitude in 0-360 convention, NOT -180/180 — using -125 directly
# (as an earlier version of this script did) makes the regrid step silently
# fail. Converted here; everything else in the script still thinks in -180/180.
...
def extract_and_regrid(grib_path, field_match, out_grib_path):
    ...
    grid over our western-US bounds, in one wgrib2 call. Returns (ok, stderr)
    so callers can log the real wgrib2 error instead of just "it failed"."""
    new_grid_spec = f"latlon {LON_MIN_360}:{GRID_NX}:{GRID_DLON} {LAT_MIN}:{GRID_NY}:{GRID_DLAT}"
    cmd = [
        "wgrib2", grib_path,
        "-match", field_match,
    ...
    ok = result.returncode == 0 and os.path.exists(out_grib_path) and os.path.getsize(out_grib_path) > 0
...
def process_hour(date_str, cycle, fhour, workdir):
    ...
    for variant, field_match in FIELD_CANDIDATES:
        ...
        ok, err_output = extract_and_regrid(grib_path, field_match, regridded)
        if not ok:
