...
def dump_native_points(cropped_path, csv_path):
    """Dump every point of the cropped record as (time, valid, level, lon,
    lat, value) CSV rows, straight from the native grid — no interpolation
    library involved at all, so this can't hit the IPOLATES grid-recognition
    failure that -new_grid did."""
    result = subprocess.run(
        ["wgrib2", cropped_path, "-csv", csv_path],
...
def rasterize_points(csv_path):
    """Read wgrib2's native-grid CSV dump and bin the scattered points onto
    ...
    with open(csv_path, newline="") as f:
        ...
        for row in reader:
            if len(row) < 6:
                continue
            try:
                lon, lat, value = float(row[-3]), float(row[-2]), float(row[-1])
            except ValueError:
                continue
            n_rows += 1
