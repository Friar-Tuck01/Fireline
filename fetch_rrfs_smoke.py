...
# version.
FIELD_CANDIDATES = [
    ("conus", r":MASSDEN:8 m above ground:.*ave fcst:aerosol=Total aerosol:aerosol_size <2\.5e-06"),
    ("conus", r":MASSDEN:8 m above ground:.*aerosol=Particulate organic matter dry:aerosol_size <2\.5e-06"),
]
...
# Same µg/m³ bins as the NWS Smoke Forecast layer, but with per-bin alpha
# tuned so the ever-present low-level background haze (0-3) doesn't wash
# out the whole map, while real fire-driven smoke (higher bins) stands out.
COLOR_STOPS = [
    (0,   (255, 255, 163), 0),    # 0-3   — background haze, fully transparent
    (3,   (250, 209, 87), 130),   # 3-25
    (25,  (242, 150, 40), 175),   # 25-63
    (63,  (219, 84, 27), 215),    # 63-158
    (158, (140, 0, 0), 255),      # 158+  — heaviest smoke, fully opaque
]
    ...
    for days_back in range(0, 3):
        ...
        for hh in range(23, -1, -1):
            ...
            check_url = (
                ...
                f"rrfs.t{cycle}z.2dfld.3km.f084.conus.grib2"
            )
...
def download(url, dest, timeout=600):
    # CONUS domain files are ~300-360MB (vs the ~25MB PR/HI nest files we
    # were mistakenly using before), so this needs a much longer timeout.
...
def crop_to_region(grib_path, field_match, cropped_path):
    ...
    Per wgrib2's own docs, -small_grib silently falls back to writing the
    FULL, uncropped grid (no error) if it hits an unsupported grid type —
    so a successful return code here doesn't guarantee the crop worked.
    We check the actual output grid dimensions to confirm."""
    cmd = [
        "wgrib2", grib_path,
        "-match", field_match,
    ...
    if ok:
        grid_check = subprocess.run(
            ["wgrib2", cropped_path, "-grid"], capture_output=True, text=True, timeout=30
        )
        print(f"    cropped grid info: {grid_check.stdout.strip()[:300]}")
...
def rasterize_points(csv_path):
    ...
    n_rows = 0
    n_in_bounds = 0
    lon_min_seen = lat_min_seen = float("inf")
    lon_max_seen = lat_max_seen = float("-inf")
    with open(csv_path, newline="") as f:
        ...
        for row in reader:
            ...
            try:
                ...
                # GRIB2's MASSDEN is in kg/m³; our color thresholds (and the
                # NWS Smoke Forecast layer's own scale) are in µg/m³.
                # Without this, every value is a tiny fraction and all land
                # in the same lowest color bin — which is exactly why the
                # rendered overlay came out as one flat, uniform color.
                value *= 1e9
            except ValueError:
                continue
            n_rows += 1
            ...
            if lon > 180:
                lon -= 360
            lon_min_seen = min(lon_min_seen, lon); lon_max_seen = max(lon_max_seen, lon)
            lat_min_seen = min(lat_min_seen, lat); lat_max_seen = max(lat_max_seen, lat)
    ...
    print(f"    actual coordinate range seen: lon {lon_min_seen:.2f} to {lon_max_seen:.2f}, "
          f"lat {lat_min_seen:.2f} to {lat_max_seen:.2f} "
          f"(target was lon {LON_MIN} to {LON_MAX}, lat {LAT_MIN} to {LAT_MAX})")
    if n_rows == 0:
        return None
    values = []
...
def value_to_color(v):
    ...
    for i in range(len(COLOR_STOPS) - 1, -1, -1):
        threshold, color, alpha = COLOR_STOPS[i]
        if v >= threshold:
            return (*color, alpha)
    return (0, 0, 0, 0)
...
def process_hour(date_str, cycle, fhour, workdir):
    ...
    for variant, field_match in FIELD_CANDIDATES:
        url = (
            f"{NOMADS_BASE}/rrfs.{date_str}/{cycle}/"
            f"rrfs.t{cycle}z.2dfld.3km.f{fhour_str}.{variant}.grib2"
        )
        grib_path = f"{workdir}/f{fhour_str}.{variant}.grib2"
        try:
            if not os.path.exists(grib_path):
                print(f"  Downloading {variant} file (this is ~300MB+, may take a bit)...")
        ...
        result = {"hour": fhour, "file": f"f{fhour_str}.png", "variant": variant, "field": field_match}
        break

    # These CONUS files are ~300-360MB each — with 15 forecast hours per run,
    # keeping them all on disk at once would use ~5GB. Clean up as we go.
    for pattern in (f"f{fhour_str}.*.grib2", f"f{fhour_str}.cropped.grib2", f"f{fhour_str}.csv"):
        for path in glob.glob(f"{workdir}/{pattern}"):
            try:
                os.remove(path)
            except OSError:
                pass
