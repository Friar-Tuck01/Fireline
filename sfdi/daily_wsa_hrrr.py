...
def run():
    ...
    valid = valid & _land_mask(rows, cols, south, west, north, east)
...
def _land_mask(rows, cols, south, west, north, east):
    """True over land, False over water, on the 450 m terrain grid.

    build_terrain marks a cell valid whenever its source tile downloaded, and
    the AWS skadi tiles cover the ocean too -- filled with ETOPO bathymetry.
    So the seafloor has perfectly good slope and aspect, the wind blows
    across it, and the layer drew alignment bands out past the continental
    shelf. Every one of those cells was computed correctly and none of them
    should exist.

    Rather than re-download 437 tiles to carry an elevation mask, reuse
    climate_class.npy, which is NaN over water on the 4 km grid. It is the
    same mask SFDI and HDWI already use, which is exactly why those layers
    stop at the coastline and this one did not. Nearest-neighbour is right
    here: a 4 km mask cannot resolve a 450 m coastline either way, and
    erring toward the existing land definition keeps the four layers
    agreeing about where the shore is.
    """
    cc = np.load(os.path.join(bc.META, "climate_class.npy"))
    glat = np.load(os.path.join(bc.META, "lat.npy"))[:, 0]
    glon = np.load(os.path.join(bc.META, "lons.npy"))
    tlat = np.linspace(north - 0.5 / 240, south + 0.5 / 240, rows)
    tlon = np.linspace(west + 0.5 / 240, east - 0.5 / 240, cols)
    ri = np.abs(glat[None, :] - tlat[:, None]).argmin(axis=1)
    ci = np.abs(glon[None, :] - tlon[:, None]).argmin(axis=1)
    land = np.isfinite(cc)[np.ix_(ri, ci)]
    print(f"   land mask: {100*land.mean():.1f}% of the grid is land "
          f"({int(land.sum()):,} of {land.size:,} cells)")
    return land
