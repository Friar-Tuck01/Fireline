...
def _update_dead_moisture(st, tmax_f, tmin_f, rhmax, rhmin,
    ...
    # The 100/1000-hour calculations continue uninterrupted regardless.
    raining = ppt_in > 0.01
...
def _update_live_moisture(st, mc1, mc1000, prev_mc1000, tmax_f, tmin_f):
    ...
    # X1000: a damped version of MC1000 that drives the live models.
    diff = mc1000 - prev_mc1000
    kwet = np.select(
        [mc1000 > 25.0, mc1000 >= 10.0],
        [1.0, 0.0333 * mc1000 + 0.1675],
        default=0.5,
    )
    kwet = np.where(diff <= 0.0, 1.0, kwet)   # drying is never damped
    ktmp = np.where((tmax_f + tmin_f) / 2.0 <= 50.0, 0.6, 1.0)
    st.x1000 = st.x1000 + diff * kwet * ktmp
...
def step(st, wx, fm=FUEL_G):
    ...
    # Wet-fuel rule: components go to zero, but the fuel moisture state
    # keeps integrating. Snow is not modeled here -- gridMET has no snow
    # flag -- so this triggers on precipitation only.
    wet = wx["ppt_in"] > 0.01
